...
describe('auth', () => {
  ...
  describe('loadTokens', () => {
    it('returns parsed JSON when file exists', () => {
      ...
      fsMock.readFileSync.mock.mockImplementation(() => '{"access_token":"abc"}');
  ...
  describe('refreshAccessToken', () => {
    it('returns new tokens on success', async () => {
      ...
      const result = await auth.refreshAccessToken({ refresh_token: 'old-refresh' });
  ...
  describe('requestDeviceAuthorization', () => {
    ...
    it('sends correct client_id and scope', async () => {
      ...
      mock.method(globalThis, 'fetch', async (_url, opts) => {
        ...
        return { ok: true, json: async () => ({ device_code: 'x', user_code: 'Y' }) };
      });

      await auth.requestDeviceAuthorization();
      assert.equal(capturedBody.get('client_id'), '1617757b-9d58-44c5-bf90-31ccd8258891');
      assert.equal(capturedBody.get('scope'), 'agent:access email openid orders profile');
    });
  ...
  describe('pollForDeviceToken', () => {
    ...
    it('returns tokens on immediate success', async () => {
      const tokens = { access_token: 'tok', refresh_token: 'ref', scope: 'openid' };
      mock.method(globalThis, 'fetch', async () => ({
        ok: true,
        json: async () => tokens,
      }));

      const result = await auth.pollForDeviceToken('dev-123', { interval: 0, expiresIn: 10 });
    ...
    it('polls through authorization_pending then succeeds', async () => {
      ...
      const result = await auth.pollForDeviceToken('dev-123', { interval: 0, expiresIn: 60 });
      assert.deepEqual(result, tokens);
  ...
  describe('stampExpiry', () => {
    it('adds expires_at from expires_in', () => {
      const now = Date.now();
      const result = auth.stampExpiry({ access_token: 'tok', expires_in: 3600 });
    ...
    it('preserves all original fields', () => {
      const result = auth.stampExpiry({ access_token: 'tok', refresh_token: 'ref', scope: 'openid' });
  ...
  describe('getValidToken', () => {
    ...
    it('returns token when valid', async () => {
      const userinfo = { email: 'user@example.com' };
      fsMock.existsSync.mock.mockImplementation(() => true);
      fsMock.readFileSync.mock.mockImplementation(() => JSON.stringify({ access_token: 'valid' }));
      mock.method(globalThis, 'fetch', async () => ({
        ok: true,
        json: async () => userinfo,
      }));
    ...
    it('returns cached userinfo when expires_at is in the future', async () => {
      ...
      mock.method(globalThis, 'fetch', async () => {
        fetchCalled = true;
        return { ok: true, json: async () => ({ email: 'network@example.com' }) };
      });
    ...
    it('refreshes expired token', async () => {
      ...
      fsMock.readFileSync.mock.mockImplementation(() =>
        JSON.stringify({ access_token: 'expired', refresh_token: 'refresh-123' })
      );
